...
Rotation::Rotation(const cv::Mat &data, RotationType t)
{
	...
	switch (t) {
	case QUAT: SetQuaternion(data); break;
	case MAT: SetMatrix(data); break;
	case EUL: SetEuler(data); break;
	case ROD: SetRodriques(data); break;
	}
}
...
void
Rotation::MirrorMat(cv::Mat &mat, bool x, bool y, bool z)
{
...
void
Rotation::Mirror(bool x, bool y, bool z)
{
	double  tmp[9];
	cv::Mat tmp_mat = cv::Mat(3, 3, CV_64F, tmp);
	GetMatrix(tmp_mat);
...
void
Rotation::Mat9ToRod(double *mat, double *rod)
{
	cv::Mat mat_m, rod_m;
...
void
Rotation::RodToMat9(const double *rod, double *mat)
{
	cv::Mat mat_m, rod_m;
	mat_m = cv::Mat(3, 3, CV_64F, mat);
...
void
Rotation::QuatInv(const double *q, double *qi)
{
	qi[0] = q[0];
	qi[1] = -1 * q[1];
	qi[2] = -1 * q[2];
...
void
Rotation::QuatMul(const double *q1, const double *q2, double *q3)
{
	double w1 = q1[0];
	double x1 = q1[1];
	double y1 = q1[2];
...
void
Rotation::QuatToMat9(const double *quat, double *mat)
{
	double W = quat[0];
	double X = quat[1];
	double Y = quat[2];
...
void
Rotation::QuatToMat16(const double *quat, double *mat)
{
...
void
Rotation::QuatToEul(const double *q, double *eul)
{
	double qw = q[0];
	double qx = q[1];
	double qy = q[2];
...
void
Rotation::Mat9ToQuat(const double *mat, double *quat)
{
...
void
Rotation::EulToQuat(const double *eul, double *quat)
{
...
void
Rotation::SetQuaternion(const cv::Mat &mat)
{
...
void
Rotation::SetQuaternion(const double *quat)
{
	quaternion[0] = quat[0];
	quaternion[1] = quat[1];
	quaternion[2] = quat[2];
...
void
Rotation::SetEuler(const cv::Mat &mat)
{
	EulToQuat(mat.ptr<double>(0), quaternion);
}

void
Rotation::SetRodriques(const cv::Mat &mat)
{
	double tmp[9];
...
void
Rotation::SetMatrix(const cv::Mat &mat)
{
	double tmp[9];
	for (int i = 0; i < 3; ++i)
		for (int j = 0; j < 3; ++j)
...
void
Rotation::GetMatrix(cv::Mat &mat) const
{
	if (mat.cols == 3) {
...
void
Rotation::GetRodriques(cv::Mat &mat) const
{
	double tmp[9];
...
void
Rotation::GetEuler(cv::Mat &mat) const
{
	QuatToEul(quaternion, mat.ptr<double>(0));
}

void
Rotation::GetQuaternion(cv::Mat &mat) const
{

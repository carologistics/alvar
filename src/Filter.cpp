...
void
FilterAverage::push_to_buffer(double y)
{
	buffer.push_back(y);
...
double
FilterRunningAverage::next(double y)
{
	if (breset) {
		breset = false;
		value  = (double)y;
...
void
FilterRunningAverage::reset()
{
	breset = true;
}

double
FilterDoubleExponentialSmoothing::next(double y)
{
	if (breset) {
		breset = false;
		value  = (double)y;

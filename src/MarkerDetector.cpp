...
void
MarkerDetectorImpl::TrackMarkersReset()
{
	_track_markers_clear();
}

void
MarkerDetectorImpl::TrackMarkerAdd(int id, PointDouble corners[4])
{
...
void
MarkerDetectorImpl::SetMarkerSize(double _edge_length, int _res, double _margin)
{
...
void
MarkerDetectorImpl::SetMarkerSizeForId(unsigned long id, double _edge_length)
{
	map_edge_length[id] = _edge_length;
}

void
MarkerDetectorImpl::SetOptions(bool _detect_pose_grayscale)
{
	detect_pose_grayscale = _detect_pose_grayscale;
}

int
MarkerDetectorImpl::Detect(cv::Mat &      image,
                           Camera *       cam,
                           bool           track,
                           bool           visualize,
                           ...
                           bool           update_pose)
{
	...
	for (size_t i = 0; i < blob_corners.size(); ++i) {
		...
		Marker *mn = new_M(edge_length, res, margin);
		if (mn->UpdateContent(blob_corners[i], gray, cam) && mn->DecodeContent(&orientation)
...
int
MarkerDetectorImpl::DetectAdditional(cv::Mat &image,
                                     Camera * cam,
                                     bool     visualize,
                                     double   max_track_error)
{

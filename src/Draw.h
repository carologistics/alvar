...
void inline DrawBB(cv::Mat &                     image,
                   ...
                   const std::string &           label = "")
{
	...
	cv::line(image,
	         cv::Point((int)minimum.x, (int)minimum.y),
	         cv::Point((int)maximum.x, (int)minimum.y),
	         color);
	cv::line(image,
	         cv::Point((int)maximum.x, (int)minimum.y),
	         cv::Point((int)maximum.x, (int)maximum.y),
	         color);
	cv::line(image,
	         cv::Point((int)maximum.x, (int)maximum.y),
	         cv::Point((int)minimum.x, (int)maximum.y),
	         color);
	cv::line(image,
	         cv::Point((int)minimum.x, (int)maximum.y),
	         cv::Point((int)minimum.x, (int)minimum.y),
	...
	if (!label.empty()) {
		cv::putText(
		  image, label, cv::Point((int)minimum.x + 1, (int)minimum.y + 2), 0, 0.5, CV_RGB(255, 255, 0));
	}
}
...
*/
template <class PointType>
void inline DrawLines(cv::Mat &                     image,
                      const std::vector<PointType> &points,
                      const cv::Scalar &            color,
                      bool                          loop = true)
{
	for (unsigned i = 1; i < points.size(); ++i)
		cv::line(image,
		         cv::Point((int)points[i - 1].x, (int)points[i - 1].y),
		         cv::Point((int)points[i].x, (int)points[i].y),
		         color);
	if (loop) {
		cv::line(image,
		         cv::Point((int)points[points.size() - 1].x, (int)points[points.size() - 1].y),
		         cv::Point((int)points[0].x, (int)points[0].y),
		         color);
	}
}
...
/** \brief			Draws a line.
  ...
  * \param color	Use CV_RGB(red,green,blue) to determine the color.
*/
void ALVAR_EXPORT DrawLine(cv::Mat &         image,
                           const Line        line,
...
*/
void ALVAR_EXPORT DrawCVEllipse(cv::Mat &              image,
 ...
 */
void ALVAR_EXPORT BuildHideTexture(cv::Mat &   image,
                                   cv::Mat &   hide_texture,
                                   Camera *    cam,
 ...
 *  \param botright		Bottom right limit of the texture area in marker coordinate frame. 
 */
void ALVAR_EXPORT DrawTexture(cv::Mat &   image,
                              cv::Mat &   texture,
                              Camera *    cam,
                              double      gl_modelview[16],
                              PointDouble topleft,

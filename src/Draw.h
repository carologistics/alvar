...
void inline DrawBB(cv::Mat&image, const std::vector<PointType>& points, CvScalar color, std::string label="")
{
    ...
    if (!label.empty()) {
		...
		cvPutText(image, label.c_str(), cv::Point((int)minimum.x+1,(int)minimum.y+2), &font, cvScalar(CV_RGB(255,255,0)));
    }
}

/** \brief Draws a set of points.
 *  \param image Pointer to the destination image.
 *  \param points Array of cv::Points to be visualzed.
 *  \param color Use CV_RGB(red,green,blue) to determine the color.
*/
void ALVAR_EXPORT DrawPoints(cv::Mat&image, const std::vector<cv::Point>& points, CvScalar color);
...
/** \brief			Draws a line.
  ...
  * \param color	Use CV_RGB(red,green,blue) to determine the color.
*/
void ALVAR_EXPORT DrawLine(cv::Mat& image, const Line line, CvScalar color = cvScalar(CV_RGB(0,255,0)));
...
/** \brief Draws points of the contour that is obtained by \e Labeling class.
  ...
  * \param color	Use CV_RGB(red,green,blue) to determine the color.
*/
void ALVAR_EXPORT DrawPoints(cv::Mat& image, const CvSeq* contour, CvScalar color = cvScalar(CV_RGB(255,0,0)));
...
/** \brief Draws circles to the contour points that are obtained by \e Labeling class.
  ...
  * \param color	Use CV_RGB(red,green,blue) to determine the color.
*/
void ALVAR_EXPORT DrawCircles(cv::Mat& image, const CvSeq* contour, int radius, CvScalar color = cvScalar(CV_RGB(255,0,0)));
...
/** \brief Draws lines between consecutive contour points.
  ...
  * \param color	Use CV_RGB(red,green,blue) to determine the color.
*/
void ALVAR_EXPORT DrawLines(cv::Mat& image, const CvSeq* contour, CvScalar color = cvScalar(CV_RGB(255,0,0)));
...
*/
template<class PointType>
void inline DrawPoints(cv::Mat&image, const std::vector<PointType>& points, CvScalar color, int radius=1)
{
	for(unsigned i = 0; i < points.size(); ++i)
		cvCircle(image, cv::Point((int)points[i].x,(int)points[i].y), radius, color);
}
...
*/
void ALVAR_EXPORT DrawCVEllipse(cv::Mat& image, CvBox2D& ellipse, CvScalar color, bool fill=false, double par=0);

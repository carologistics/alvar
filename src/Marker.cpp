...
void
Marker::VisualizeMarkerPose(cv::Mat &        image,
                            Camera *         cam,
...
void
Marker::VisualizeMarkerContent(cv::Mat &image,
                               Camera * cam,
                               ...
                               double   content_point[2]) const
{
	...
	for (size_t i = 0; i < marker_allpoints_img.size(); i++) {
		if (marker_allpoints_img[i].val == 0)
			cv::circle(image,
			           cv::Point(int(marker_allpoints_img[i].x), int(marker_allpoints_img[i].y)),
			           1,
			           CV_RGB(0, 255, 0));
	...
	cv::putText(image,
	            val.str(),
	            cv::Point((int)datatext_point[0], (int)datatext_point[1]),
	            0,
	            0.5,
	            CV_RGB(255, 255, 0));
...
void
Marker::VisualizeMarkerError(cv::Mat &image, Camera *cam, double errortext_point[2]) const
{
	std::stringstream val;
	if (GetError(MARGIN_ERROR | DECODE_ERROR) > 0) {
		...
		cv::putText(image,
		            val.str(),
		            cv::Point((int)errortext_point[0], (int)errortext_point[1]),
		            0,
		            0.5,
		            CV_RGB(255, 0, 0));
...
void
MarkerData::VisualizeMarkerContent(cv::Mat &image,
                                   Camera * cam,
                                   double   datatext_point[2],
                                   double   content_point[2]) const
{
	...
	cv::putText(
	  image, val.str(), cv::Point((int)datatext_point[0], (int)datatext_point[1]), 0, 0.5, rgb);
}

void
Marker::Visualize(cv::Mat &image, Camera *cam, const cv::Scalar color) const
{
...
void
Marker::CompareCorners(vector<PointDouble> &_marker_corners_img, int *orientation, double *error)
{
...
void
Marker::CompareContent(vector<PointDouble> &_marker_corners_img,
                       cv::Mat &            gray,
                       Camera *             cam,
...
bool
Marker::UpdateContent(vector<PointDouble> &_marker_corners_img,
                      cv::Mat &            gray,
                      Camera *             cam,
                      int                  frame_no /*= 0*/)
{
...
bool
Marker::UpdateContentBasic(vector<PointDouble> &_marker_corners_img,
                           cv::Mat &            gray,
                           Camera *             cam,
                           int                  frame_no /*= 0*/)
{
...
void
Marker::UpdatePose(vector<PointDouble> &_marker_corners_img,
                   Camera *             cam,
                   int                  orientation,
                   int                  frame_no /* =0 */,
...
bool
Marker::DecodeContent(int *orientation)
{
	*orientation = 0;
	decode_error = 0;
	return true;
...
void
Marker::SaveMarkerImage(const char *filename, int save_res) const
{
	double scale;
	if (save_res == 0) {
	...
	cv::Mat submat =
	  img(cv::Rect(int(margin * scale), int(margin * scale), int(res * scale), int(res * scale)));
...
void
Marker::ScaleMarkerToImage(cv::Mat &image) const
{
	...
	cv::Mat   img        = cv::Mat::zeros(cv::Size(int(multiplier * (res + margin + margin) + 0.5),
                               ...
                               CV_8UC1);
	cv::Mat   img_content =
	  cv::Mat(cv::Size(int(multiplier * res + 0.5), int(multiplier * res + 0.5)), CV_8UC1);
	cv::Mat submat = img(cv::Rect(int(multiplier * margin + 0.5),
	                              int(multiplier * margin + 0.5),
	                              int(multiplier * res + 0.5),
	                              int(multiplier * res + 0.5)));
	cv::resize(marker_content, img_content, img_content.size(), 0, 0, cv::INTER_NEAREST);
	img_content.copyTo(submat);
...
void
Marker::SetMarkerSize(double _edge_length, int _res, double _margin)
{
...
bool
MarkerArtoolkit::DecodeContent(int *orientation)
{
...
void
MarkerArtoolkit::SetContent(unsigned long _id)
{
...
void
MarkerData::DecodeOrientation(int *error, int *total, int *orientation)
{
	int            i, j;
	vector<double> errors(4);
	int            color = 255;
...
bool
MarkerData::DetectResolution(vector<PointDouble> &_marker_corners_img, cv::Mat &gray, Camera *cam)
{
	vector<PointDouble> marker_corners_img_undist;
	marker_corners_img_undist.resize(_marker_corners_img.size());
	copy(_marker_corners_img.begin(), _marker_corners_img.end(), marker_corners_img_undist.begin());
...
bool
MarkerData::UpdateContent(vector<PointDouble> &_marker_corners_img,
                          cv::Mat &            gray,
                          Camera *             cam,
                          int                  frame_no /*= 0*/)
{
...
int
MarkerData::DecodeCode(int            orientation,
                       BitsetExt *    bs,
                       int *          erroneous,
                       int *          total,
...
void
MarkerData::Read6bitStr(BitsetExt *bs, char *s, size_t s_max_len)
{
...
bool
MarkerData::DecodeContent(int *orientation)
{
...
void
MarkerData::Add6bitStr(BitsetExt *bs, char *s)
{
	while (*s) {
...
int
MarkerData::UsableDataBits(int marker_res, int hamming)
{
	if (marker_res < 5)
		return 0;
	if (!(marker_res % 2))
...
void
MarkerData::SetContent(MarkerContentType _content_type,
                       unsigned long     _id,
                       const char *      _str,

...
{
protected:
	void         VisualizeMarkerPose(cv::Mat &        image,
	                                 Camera *         cam,
	                                 double           visualize2d_points[12][2],
	                                 const cv::Scalar color = CV_RGB(255, 0, 0)) const;
	virtual void VisualizeMarkerContent(cv::Mat &image,
	                                    Camera * cam,
...
public:
	 ...
	 */
	virtual bool UpdateContent(std::vector<Point<cv::Point2d>> &_marker_corners_img,
	                           cv::Mat &                        gray,
	                           Camera *                         cam,
	                           int                              frame_no = 0);
	/** \brief Updates the markers \e pose estimation
	 */
	void UpdatePose(std::vector<Point<cv::Point2d>> &_marker_corners_img,
	                Camera *                         cam,
	                int                              orientation,
	                int                              frame_no    = 0,
	...
	*/
	 ...
	 */
	double
	GetError(int errors = (MARGIN_ERROR | DECODE_ERROR)) const
	{
		int    count = 0;
		double error = 0;
	...
	void
	SetError(int error_type, double value)
	{
...
protected:
	...
	int          DecodeCode(int            orientation,
	                        BitsetExt *    bs,
	                        int *          erroneous,
	                        int *          total,
	...
	bool         DetectResolution(std::vector<Point<cv::Point2d>> &_marker_corners_img,
	                              cv::Mat &                        gray,
	                              Camera *                         cam);
...
public:
	...
	*/
	...
	/** \brief Updates the \e marker_content by "encoding" the given parameters
	 */
	void SetContent(MarkerContentType content_type,
	                unsigned long     id,
	                const char *      str,
...
public:
	...
	MarkerIteratorImpl &
	operator=(const MarkerIteratorImpl &other)
	{
		_i = other._i;
		return (*this);
	}
	...
	bool
	operator==(const MarkerIterator &other)
	{

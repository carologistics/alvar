...
void
TrackerStat::Compensate(double *x, double *y)
{
	*x += xd;
	*y += yd;
}
...
TrackerStatRot::Track(cv::Mat &img)
{
	...
	for (int p = 0; p < f.prev_feature_count; p++) {
		for (int c = 0; c < f.feature_count; c++) {
			...
			double theta_pred =
			  atan2((double)y_pred - (y_res / 2), (double)x_pred - (x_res / 2)) * 180.0 / 3.1415926535;
			double theta_curr =
			  atan2((double)y_curr - (y_res / 2), (double)x_curr - (x_res / 2)) * 180.0 / 3.1415926535;
...
void
TrackerStatRot::Compensate(double *x, double *y)
{
	double xx     = *x - (x_res / 2);
	double yy     = *y - (y_res / 2);

...
MultiMarkerFiltered::MultiMarkerFiltered(std::vector<int> &indices) : MultiMarker(indices)
{
...
void
MultiMarkerFiltered::PointCloudAverage(int marker_id, double edge_length, Pose &pose)
{
	if (marker_id == 0) {
...
double
MultiMarkerFiltered::_Update(MarkerIterator &begin,
                             MarkerIterator &end,
                             Camera *        cam,
                             Pose &          pose,
                             cv::Mat &       image)
{
	if (_GetPose(begin, end, cam, pose, image) == -1)
		return -1;

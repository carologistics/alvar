...
int
dot(const cv::Point &A, const cv::Point &B, const cv::Point &C)
{
	cv::Point AB, BC;
	AB.x    = B.x - A.x;
	AB.y    = B.y - A.y;
...
int
cross(const cv::Point &A, const cv::Point &B, const cv::Point &C)
{
	cv::Point AB, AC;
	AB.x      = B.x - A.x;
	AB.y      = B.y - A.y;
...
double
distance(const cv::Point &A, const cv::Point &B)
{
	double d1 = A.x - B.x;
	double d2 = A.y - B.y;
...
double
linePointDist(const cv::Point &A, const cv::Point &B, const cv::Point &C, bool isSegment)
{
...
double
polyLinePointDist(const std::vector<cv::Point> &points,
                  const cv::Point &             C,
                  int *                         index,
                  int                           isClosedPolygon)
{
...
void
FitCVEllipse(const vector<PointDouble> &points, cv::RotatedRect &ellipse_box)
{
	if (points.size() < 8)
		return;
...
int
exp_filt2(vector<double> &v, vector<double> &ret, bool clamp)
{
	double y;
	int    n = (int)v.size();
...
int
find_zero_crossings(const vector<double> &v, vector<int> &corners, int offs)
{
	int ind = 0;
	int len = (int)v.size();
...
void
out_matrix(const cv::Mat &m, const char *name)
{
	if (m.cols == 1) {
		std::cout << name << " = [";
...
double
Limit(double val, double min_val, double max_val)
{
...
int
Histogram::DimIndex(int dim, double val)
{
...
double
Histogram::DimVal(int dim, int index)
{
	int binsize = (dim < int(dim_binsize.size()) ? dim_binsize[dim] : 1);
	return (index * binsize);
}
void
Histogram::AddDimension(int binsize)
{
...
void
Histogram::Inc(double dim0, double dim1, double dim2)
{
...
int
Histogram::GetMax(double *dim0, double *dim1, double *dim2)
{
...
void
HistogramSubpixel::Inc(double dim0, double dim1, double dim2)
{
	Index index(DimIndex(0, dim0), DimIndex(1, dim1), DimIndex(2, dim2));
	if (bins.find(index) != bins.end()) {
		bins[index]++;
...
int
HistogramSubpixel::GetMax(double *dim0, double *dim1, double *dim2)
{
...
bool
Serialization::Descend(const char *id)
{
	SerializationFormatterXml *xml = (SerializationFormatterXml *)formatter_handle;
	if (input) {
...
bool
Serialization::Serialize(int &data, const std::string &name)
{
	SerializationFormatterXml *xml = (SerializationFormatterXml *)formatter_handle;
...
bool
Serialization::Serialize(unsigned short &data, const std::string &name)
{
	int  i   = data;
...
bool
Serialization::Serialize(unsigned long &data, const std::string &name)
{
...
bool
Serialization::Serialize(double &data, const std::string &name)
{
	SerializationFormatterXml *xml = (SerializationFormatterXml *)formatter_handle;
	bool                       ret = true;
	if (input)
...
bool
Serialization::Serialize(std::string &data, const std::string &name)
{
	SerializationFormatterXml *xml = (SerializationFormatterXml *)formatter_handle;
	bool                       ret = true;
	if (input) {
...
bool
Serialization::Serialize(cv::Mat &data, const std::string &name)
{
	SerializationFormatterXml *xml = (SerializationFormatterXml *)formatter_handle;
	bool                       ret = true;
	if (input) {

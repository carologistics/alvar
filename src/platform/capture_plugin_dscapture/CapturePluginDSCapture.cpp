...
CaptureDSCapture::start()
{
	if (m_pDSCapture) {
		...
		mReturnFrame =
		  cvCreateImageHeader(cvSize(m_nVideo_x_res, m_nVideo_y_res), IPL_DEPTH_8U, m_nBpp / 8);
...
bool
CaptureDSCapture::showSettingsDialog()
{
...
string
CaptureDSCapture::SerializeId()
{
	return "CaptureDSCapture";
}

bool
CaptureDSCapture::Serialize(Serialization *serialization)
{
	return false;
}

void
CaptureDSCapture::OnVideoSample(BYTE *pBuffer, DWORD dwDataLen, REFERENCE_TIME t_start)
{
	EnterCriticalSection(&crit);
	if (pBuffer) {
...
CapturePluginDSCapture::enumerateDevices()
{
...
Capture *
CapturePluginDSCapture::createCapture(const CaptureDevice captureDevice)
{
	return new CaptureDSCapture(captureDevice);
}

void
registerPlugin(const string &captureType, alvar::CapturePlugin *&capturePlugin)
{

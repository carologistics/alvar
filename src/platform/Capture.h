...
public:
	...
	: mCaptureDevice(captureDevice), mXResolution(0), mYResolution(0), mIsCapturing(false)
	{
	}
     ...
     */
	virtual void
	setResolution(const unsigned long xResolution, const unsigned long yResolution)
	{
	}
     ...
     */
	virtual bool
	loadSettings(std::string filename)
	{
		if (!isCapturing()) {
			return false;
		}

...
CaptureFactoryPrivate::~CaptureFactoryPrivate()
{
	for (CapturePluginMap::iterator itr = mCapturePluginMap.begin(); itr != mCapturePluginMap.end();
	     itr++) {
		delete itr->second;
	}
...
void
CaptureFactoryPrivate::loadPlugins()
{
...
void
CaptureFactoryPrivate::loadPlugin(const std::string &captureType)
{
...
void
CaptureFactoryPrivate::loadPlugin(const std::string &captureType, const std::string &filename)
{
	// ensure plugin is not alredy loaded
	if (mPluginMap.find(captureType) != mPluginMap.end()) {
		return;
...
CapturePlugin *
CaptureFactoryPrivate::getPlugin(const std::string &captureType)
{
...
CaptureFactory::enumeratePlugins()
{
	// load all plugins
	d->loadPlugins();
...
CaptureFactory::enumerateDevices(const std::string &captureType)
{
	...
	if (captureType.empty()) {
		d->loadPlugins();
		for (CaptureFactoryPrivate::CapturePluginMap::iterator itr = d->mCapturePluginMap.begin();
		     itr != d->mCapturePluginMap.end();
		     ++itr) {
...
Capture *
CaptureFactory::createCapture(const CaptureDevice captureDevice)
{

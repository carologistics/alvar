...
: Capture(captureDevice), mCamera(new C1394Camera()), mChannels(-1), mReturnFrame(NULL)
{
}
...
std::string
CaptureCmu::SerializeId()
{
	return "CaptureCmu";
}

bool
CaptureCmu::Serialize(Serialization *serialization)
{
	if (!mCamera) {
		return false;
	}
...
CapturePluginCmu::CapturePluginCmu(const std::string &captureType) : CapturePlugin(captureType)
{
}
...
CapturePluginCmu::enumerateDevices()
{
...
Capture *
CapturePluginCmu::createCapture(const CaptureDevice captureDevice)
{

...
int
MultiMarker::pointcloud_index(int marker_id, int marker_corner, bool add_if_missing /*=false*/)
{
	return (get_id_index(marker_id, add_if_missing) * 4) + marker_corner;
}

int
MultiMarker::get_id_index(int id, bool add_if_missing /*=false*/)
{
...
bool
MultiMarker::SaveXML(const char *fname)
{
...
bool
MultiMarker::SaveText(const char *fname)
{
...
bool
MultiMarker::Save(const char *fname, FILE_FORMAT format)
{
	switch (format) {
	case FILE_FORMAT_XML: return SaveXML(fname);
	case FILE_FORMAT_TEXT:
	case FILE_FORMAT_DEFAULT: return SaveText(fname);
	default: return false;
	}
}

bool
MultiMarker::LoadXML(const char *fname)
{
	TiXmlDocument document;
...
bool
MultiMarker::LoadText(const char *fname)
{
...
bool
MultiMarker::Load(const char *fname, FILE_FORMAT format)
{
	switch (format) {
	case FILE_FORMAT_XML: return LoadXML(fname);
	case FILE_FORMAT_TEXT:
	case FILE_FORMAT_DEFAULT: return LoadText(fname);
	default: return false;
	}
}
...
void
MultiMarker::PointCloudReset()
{
	pointcloud.clear();
}

void
MultiMarker::PointCloudCorners3d(double edge_length, Pose &pose, cv::Point3d corners[4])
{
...
void
MultiMarker::PointCloudAdd(int marker_id, double edge_length, Pose &pose)
{
...
void
MultiMarker::PointCloudCopy(const MultiMarker *m)
{
	pointcloud.clear();
...
void
MultiMarker::PointCloudGet(int marker_id, int point, double &x, double &y, double &z)
{
...
bool
MultiMarker::IsValidMarker(int marker_id)
{
...
double
MultiMarker::_GetPose(MarkerIterator &begin,
                      MarkerIterator &end,
                      Camera *        cam,
                      Pose &          pose,
                      cv::Mat &       image)
	...
	for (MarkerIterator &i = begin.reset(); i != end; ++i) {
		...
		if (marker_status[index] > 0) {
			for (size_t j = 0; j < marker->marker_corners.size(); ++j) {
				...
				if (!image.empty())
					cv::circle(image,
					           cv::Point(int(marker->marker_corners_img[j].x),
					                     int(marker->marker_corners_img[j].y)),
					           3,
					           CV_RGB(0, 255, 0));
			}
...
int
MultiMarker::_SetTrackMarkers(MarkerDetectorImpl &marker_detector,
                              Camera *            cam,
                              Pose &              pose,
                              cv::Mat &           image)
{

...
public:
	...
	cv::Point3f p3d;
	cv::Point2f
	  projected_p2d; // This is only temporary -- user needs to take care that it has valid content
...
inline int
EraseItemsEC(std::map<int, T> &container, F do_erase_test)
{
	int                                 count = 0;
...
public:
	...
	template <typename T>
	int
	EraseNonTracked(std::map<int, T> &container, int type_id = -1)
	{
	...
	double
	Reset(cv::Mat &img, cv::Mat &mask)
	{
		throw alvar::AlvarException("Method not supported");
	}
	bool
	DelFeature(int index)
	{
		throw alvar::AlvarException("Method not supported");
	}
	bool
...
public:
	...
	template <typename T>
	void
	Undistort(std::map<int, T> &container, int type_id = -1)
	{
		DoHandleTest<T> do_handle_test_default(type_id);
	...
	template <typename T, typename F>
	void
	Undistort(std::map<int, T> &container, F &do_handle_test)
	{
		typename std::map<int, T>::iterator iter     = container.begin();
		typename std::map<int, T>::iterator iter_end = container.end();
		for (; iter != iter_end; iter++) {
	...
	template <typename T>
	void
	Distort(std::map<int, T> &container, int type_id = -1)
	{
		DoHandleTest<T> do_handle_test_default(type_id);
	...
	template <typename T, typename F>
	void
	Distort(std::map<int, T> &container, F &do_handle_test)
	{
		typename std::map<int, T>::iterator iter     = container.begin();
		typename std::map<int, T>::iterator iter_end = container.end();
		for (; iter != iter_end; iter++) {
	...
	template <typename T>
	bool
	CalcExteriorOrientation(std::map<int, T> &container, Pose *pose, int type_id = -1)
	{
		DoHandleTest<T> do_handle_test_default(type_id);
	...
	template <typename T, typename F>
	bool
	CalcExteriorOrientation(std::map<int, T> &container, Pose *pose, F do_handle_test)
	{
	...
	template <typename T>
	bool
	UpdatePose(std::map<int, T> &     container,
	           Pose *                 pose,
	           int                    type_id = -1,
	...
	template <typename T>
	bool
	UpdateRotation(std::map<int, T> &container, Pose *pose, int type_id = -1)
	{
		DoHandleTest<T> do_handle_test_default(type_id);
	...
	template <typename T, typename F>
	bool
	UpdateRotation(std::map<int, T> &container, Pose *pose, F do_handle_test)
	{
		int count_points = container.size();
		if (count_points < 6)
			return false;
	...
	template <typename T>
	float
	Reproject(std::map<int, T> &container,
	          Pose *            pose,
	          int               type_id               = -1,
	...
	template <typename T>
	int
	EraseUsingReprojectionError(std::map<int, T> &container,
	...
	bool
	UpdateRotation(const cv::Mat &object_points, cv::Mat &image_points, cv::Mat &rot, cv::Mat &tra);
...
inline int
MarkerIdToContainerId(int marker_id, int corner_id, int first_id = 0, int last_id = 65535)
{
...
protected:
	template <typename T>
	bool
	PreDetect(std::pair<const int, T> &p, int type_id)
	{
		return PreDetect(p.second, type_id);
	}
	template <typename T>
	bool
	PreDetect(T &p, int type_id)
	{
...
public:
	int
	GetId(int marker_id, int corner_id, int first_id = 0, int last_id = 65535)
	{
	       ...
	       LabelingMethod    labeling_method      = CVSEQ)
	{
		...
		ret = MarkerDetector<M>::Detect(
		  image, cam, track, visualize, max_new_marker_error, max_track_error, labeling_method, false);
	...
	template <typename T>
	void
	MarkerToEC(std::map<int, T> &container,
	           int               marker_id,
	           double            edge_length,
	           Pose &            pose,
	           int               type_id  = 0,
...
public:
	...
	template <typename T>
	bool
	MarkersToEC(std::map<int, T> &container, int type_id = 0, int first_id = 0, int last_id = 65535)
	{
		bool ret = false;
	...
	template <typename T>
	bool
	MarkersFromEC(std::map<int, T> &container, int type_id = 0, int first_id = 0, int last_id = 65535)
	{
		// TODO...
		return false;
	}

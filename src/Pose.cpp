...
Pose::Pose(const cv::Mat &tra, const cv::Mat &rot, RotationType t) : Rotation(rot, t)
{
	translation_mat = cv::Mat(4, 1, CV_64F, translation);
	translation_mat.setTo(cv::Scalar::all(0));
	translation_mat.at<double>(3, 0) = 1;
...
void
Pose::SetMatrix(const cv::Mat &mat)
{
	double tmp[9];
	for (int i = 0; i < 3; ++i)
		for (int j = 0; j < 3; ++j)
...
void
Pose::GetMatrix(cv::Mat &mat) const
{
	if (mat.cols == 3) {
...
void
Pose::GetMatrixGL(double gl[16], bool mirror)
{
	if (mirror)
...
void
Pose::SetMatrixGL(double gl[16], bool mirror)
{
	double gll[16];
...
void
Pose::Mirror(bool x, bool y, bool z)
{
	double  tmp[16];
	cv::Mat tmp_mat = cv::Mat(4, 4, CV_64F, tmp);
	GetMatrix(tmp_mat);
...
void
Pose::SetTranslation(const cv::Mat &tra)
{
	translation_mat.at<double>(0, 0) = tra.at<double>(0, 0);
	translation_mat.at<double>(1, 0) = tra.at<double>(1, 0);
	translation_mat.at<double>(2, 0) = tra.at<double>(2, 0);
...
void
Pose::SetTranslation(const double *tra)
{
	translation[0] = tra[0];
	translation[1] = tra[1];
	translation[2] = tra[2];
...
void
Pose::SetTranslation(const double x, const double y, const double z)
{
	translation[0] = x;
	translation[1] = y;
	translation[2] = z;
...
void
Pose::GetTranslation(cv::Mat &tra) const
{

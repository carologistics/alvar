...
MultiMarkerBundle::MultiMarkerBundle(std::vector<int> &indices) : MultiMarker(indices)
{
	MeasurementsReset();
}
...
void
MultiMarkerBundle::MeasurementsReset()
{
...
void
Est(cv::Mat &state, cv::Mat &estimation, void *param)
{
	...
	for (int i = 0; i < n_images; ++i) {
		...
		for (int j = 0; j < n_points; ++j) {
			...
			cv::projectPoints(mat_object_points,
			                  mat_rotation_vector,
			                  mat_translation_vector,
			                  camera->calib_K,
			                  camera->calib_D,
			                  mat_proj);
...
bool
MultiMarkerBundle::Optimize(Camera *                     _cam,
                            double                       stop,
                            int                          max_iter,
                            Optimization::OptimizeMethod method)
{
	...
	for (size_t f = 0; f < frames; f++) {
		...
		// Camera pose
		cv::Mat tra = cv::Mat(3, 1, CV_64F, &(parameters_mat.at<double>(f * 7 + 0)));
		cv::Mat qua = cv::Mat(4, 1, CV_64F, &(parameters_mat.at<double>(f * 7 + 3)));
	...
	cout << "Optimizing with " << optimization_keyframes << " keyframes and " << optimization_markers
	     << " markers" << endl;
...
void
MultiMarkerBundle::_MeasurementsAdd(MarkerIterator &begin,
                                    MarkerIterator &end,

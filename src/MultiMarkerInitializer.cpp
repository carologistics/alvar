...
MultiMarkerInitializer::MultiMarkerInitializer(std::vector<int> &indices,
...
void
MultiMarkerInitializer::MeasurementsAdd(MarkerIterator &begin, MarkerIterator &end)
{
...
void
MultiMarkerInitializer::MeasurementsReset()
{
	measurements.clear();
	PointCloudReset();
...
int
MultiMarkerInitializer::Initialize(Camera *cam)
{
	for (bool found_new = true; found_new;) {
		...
		for (MeasurementIterator mi = measurements.begin(); mi != measurements.end(); ++mi) {
			...
			double                                err = _GetPose(m_begin, m_end, cam, pose);
			if (err >= 0) {
...
bool
MultiMarkerInitializer::updateMarkerPoses(vector<MarkerMeasurement> &markers, const Pose &pose)
{

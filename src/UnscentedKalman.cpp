...
UnscentedKalman::update(UnscentedObservation *obs)
{
	...
	cv::Mat predObsCovariance = cv::Mat(obs_n, obs_n, CV_64F, this->predObsCovariance->data.db);
	cv::Mat statePredObsCrossCorrelation =
	  cv::Mat(state_n, obs_n, CV_64F, this->statePredObsCrossCorrelation->data.db);
	...
	for (int i = 0; i < sigma_n; i++) {
      ...
      i == 0 ? (double)state_k / (double)(state_n + state_k) : .5 / (double)(state_n + state_k);
		...
		cvGEMM(
		  &predObsDiff, &predObsDiff, scale, &predObsCovariance, 1., &predObsCovariance, CV_GEMM_B_T);
		cvGEMM(stateDiff,
		       &predObsDiff,
		       scale,
	...
	cv::Mat invPredObsCovariance = cv::Mat(obs_n, obs_n, CV_64F, this->invPredObsCovariance->data.db);

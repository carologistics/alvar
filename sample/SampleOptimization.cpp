...
{
	double xx = random(CV_RAND_UNI,
	                   -(poly_res / 2),
	                   +(poly_res / 2)); //(rand()*poly_res/RAND_MAX)-(poly_res/2);
	double yy = get_y(xx, a, b, c, d, e);
	double ry =
	  random(CV_RAND_NORMAL, 0, poly_res / 8); //(rand()*(poly_res/4)/RAND_MAX)-(poly_res/8);
	yy += ry;
	*x = xx;
	*y = yy;
	if (*y < -(poly_res / 2))
		...
		for (int data_degree = 0; data_degree < 5; data_degree++) {
			...
			for (int i = 0; i < 1000; i++) {
				...
				if (get_measurement(&x, &y, a, b, c, d, e)) {
					...
					cv::circle(img, cv::Point(int(x), int(y)), 1, cvScalar(CV_RGB(0, 255, 0)));
				}
			}
			...
			for (int degree = 0; degree < 5; degree++) {
				...
				for (int x2 = step; x2 < res; x2 += step) {
					...
					cv::line(img,
					         cv::Point(x1, y1),
					         cv::Point(x2, y2),
					         cvScalar(CV_RGB(degree * 50, 255 - (degree * 50), 255)));
				}
				cv::imshow("SampleOptimization", img);
				cv::waitKey(10);
			}

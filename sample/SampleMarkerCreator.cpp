		...
		if (marker_type == 0) {
			...
			if (img.empty()) {
				...
				if (maxy > new_maxy)
					new_maxy = maxy;
				cv::Mat new_img =
				  cv::Mat(cv::Size(int(new_maxx - new_minx + 0.5), int(new_maxy - new_miny + 0.5)),
				          CV_8UC1,
				          cv::Scalar(255));
				img.copyTo(new_img(
				  cv::Rect(int(minx - new_minx + 0.5), int(miny - new_miny + 0.5), img.cols, img.rows)));
				img.release();
				img       = new_img;
				int roi_x = int((posx * units) - (marker_side_len * units / 2.0) - new_minx + 0.5);
				int roi_y = int((posy * units) - (marker_side_len * units / 2.0) - new_miny + 0.5);
				int roi_t = -roi_y;
			...
			img.adjustROI(roi_offset.y,
			              roi_size.height - img.rows,
			              roi_offset.x,
...
main(int argc, char *argv[])
{
	try {
		...
		if (st.prompt) {
			...
			std::cout << "  This is an example of how to use the 'MarkerData' and 'MarkerArtoolkit'"
			          << std::endl;
			std::cout << "  classes to generate marker images. This application can be used to"
			          << std::endl;
			...
			std::cout << "    -u 96             use units corresponding to 1.0 unit per 96 pixels"
			          << std::endl;
			std::cout << "    -uin              use inches as units (assuming 96 dpi)" << std::endl;
			std::cout << "    -ucm              use cm's as units (assuming 96 dpi) <default>"
			          << std::endl;
			std::cout << "    -s 5.0            use marker size 5.0x5.0 units (default 9.0x9.0)"
			          << std::endl;
			...
			std::cout << "    -p                prompt marker placements interactively from the user"
			          << std::endl;
			std::cout << std::endl;
			...
			if (st.prompt) {
				...
				std::cout << "  units: " << st.units / 96.0 * 2.54 << " cm " << st.units / 96.0 << " inches"
				          << std::endl;

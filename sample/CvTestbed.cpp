...
void
CvTestbed::default_videocallback(cv::Mat &image)
{
...
void
CvTestbed::ShowVisibleImages()
{
	for (size_t i = 0; i < images.size(); i++) {
		if (images[i].visible) {
			cv::imshow(images[i].title.c_str(), images[i].ipl);
		}
	}
}
...
void
CvTestbed::SetVideoCallback(void (*_videocallback)(cv::Mat &image))
{
	videocallback = _videocallback;
}

void
CvTestbed::SetKeyCallback(int (*_keycallback)(int key))
{
	keycallback = _keycallback;
}

bool
CvTestbed::StartVideo(Capture *_cap, const char *_wintitle)
{
	bool clean = false;
	cap        = _cap;
	if (cap == NULL) {
...
cv::Mat
CvTestbed::CreateImageWithProto(const char *title,
                                cv::Mat &   proto,
                                int         depth /* =0 */,
                                int         channels /* =0 */)
{
...
size_t
CvTestbed::GetImageIndex(const char *title)
{
	std::string s(title);
	for (size_t i = 0; i < images.size(); i++) {
...
cv::Mat
CvTestbed::GetImage(const char *title)
{
	return GetImage(GetImageIndex(title));
}

bool
CvTestbed::ToggleImageVisible(size_t index, int flags)
{
	if (index >= images.size())
		return false;

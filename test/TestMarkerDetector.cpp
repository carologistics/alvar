...
protected:
	...
	void
	PrintMatrixDiff(char *title, double *gl1, double *gl2)
	{
		cout << title << " [" << endl;
		cout << fixed << setprecision(4);
		cout << "\t" << (gl1[0]) << "\t" << (gl1[4]) << "\t" << (gl1[8]) << "\t" << (gl1[12]) << " | ";
		cout << "\t" << (gl2[0] - gl1[0]) << "\t" << (gl2[4] - gl1[4]) << "\t" << (gl2[8] - gl1[8])
		     << "\t" << (gl1[12] - gl2[12]) << endl;
		cout << "\t" << (gl1[1]) << "\t" << (gl1[5]) << "\t" << (gl1[9]) << "\t" << (gl1[13]) << " | ";
		cout << "\t" << (gl2[1] - gl1[1]) << "\t" << (gl2[5] - gl1[5]) << "\t" << (gl2[9] - gl1[9])
		     << "\t" << (gl1[13] - gl2[13]) << endl;
		cout << "\t" << (gl1[2]) << "\t" << (gl1[6]) << "\t" << (gl1[10]) << "\t" << (gl1[14]) << " | ";
		cout << "\t" << (gl2[2] - gl1[2]) << "\t" << (gl2[6] - gl1[6]) << "\t" << (gl2[10] - gl1[10])
		     << "\t" << (gl1[14] - gl2[14]) << endl;
		cout << "\t" << (gl1[3]) << "\t" << (gl1[7]) << "\t" << (gl1[11]) << "\t" << (gl1[15]) << " | ";
		cout << "\t" << (gl2[3] - gl1[3]) << "\t" << (gl2[7] - gl1[7]) << "\t" << (gl2[11] - gl1[11])
		     << "\t" << (gl1[15] - gl2[15]) << endl;
		cout << "]" << endl;
	}
	double
	RotDiff(double *gl1, double *gl2)
	{
		return sqrt((gl2[0] - gl1[0]) * (gl2[0] - gl1[0]) + (gl2[1] - gl1[1]) * (gl2[1] - gl1[1])
		            + (gl2[2] - gl1[2]) * (gl2[2] - gl1[2]) + (gl2[4] - gl1[4]) * (gl2[4] - gl1[4])
		            + (gl2[5] - gl1[5]) * (gl2[5] - gl1[5]) + (gl2[6] - gl1[6]) * (gl2[6] - gl1[6])
		            + (gl2[8] - gl1[8]) * (gl2[8] - gl1[8]) + (gl2[9] - gl1[9]) * (gl2[9] - gl1[9])
		            + (gl2[10] - gl1[10]) * (gl2[10] - gl1[10]));
	}
	double
	TraDiff(double *gl1, double *gl2)
	{
	           ...
	           GLfloat   marker_side_len)
	{
		...
		*/
		...
		if (render_mul == 1) {
			...
			IplImage *imgl =
			  cvCreateImage(cvSize(x_res * render_mul, y_res * render_mul), IPL_DEPTH_8U, 3);
	...
	void
	CheckPose(IplImage *img,
	          int       marker_id,
	          float     marker_side_len,
	          double    zNear,
	          double    zFar,
	          double    m[16],
	          double    p[16],
	          ...
	          bool      lower_left_origin = false)
	{
		...
		if (marker_detector.markers->size() == 1) {
			...
			double mm[16];
			cout << "Marker id " << marker_detector.markers->at(0).GetId() << " <-> " << marker_id
			     << endl;
			...
			if (detect_pose_grayscale) {
				...
				total_diff_count++;
				cout << "Modelview rot/tra diffs: " << rot_diff << ", " << tra_diff
				     << " (gray opt average: " << total_rot_diff / total_diff_count << ", "
				     << total_tra_diff / total_diff_count << ")" << endl;
			} else {
				static double total_tra_diff = 0, total_rot_diff = 0;
				static int    total_diff_count = 0;
				total_rot_diff += rot_diff;
				...
				total_diff_count++;
				cout << "Modelview rot/tra diffs: " << rot_diff << ", " << tra_diff
				     << " (average: " << total_rot_diff / total_diff_count << ", "
				     << total_tra_diff / total_diff_count << ")" << endl;
			}
		}
...
public:
	TestMarkerDetector()
	{
		...
		glTexImage2D(
		  GL_TEXTURE_2D, 0, GL_LUMINANCE8, 16, 16, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, pixels);
	}
	~TestMarkerDetector()
	{

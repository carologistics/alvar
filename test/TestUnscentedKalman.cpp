...
class SimpleFilter : public UnscentedKalman, public UnscentedProcess
{
public:
	CvMat *processNoise;
...
public:
	...
	testReentryVecicleTracking()
	{
		...
		for (int i = 0; i < 100000 && real_pos_x > 6374.; i++) {
			...
			double r = sqrt(pow(real_pos_x - xr, 2) + pow(real_pos_y - yr, 2)) + r_noise.gaussian(1e-3);
			double t = atan((real_pos_y - yr) / (real_pos_x - xr)) + t_noise.gaussian(.17e-3);
	  ...
	  real_pos_x, real_pos_y, real_vel_x, real_vel_y, real_coeff);
        */
				printf("  Real X: %7.2f dX: %7.4f b: %6.4f", real_pos_x, real_vel_x, real_coeff);
				double pos_x, pos_y, vel_x, vel_y, coeff;
	  ...
	  pos_x, pos_y, vel_x, vel_y, coeff);
        */
				printf(" | Est. X: %7.2f dX: %7.4f b: %6.4f", pos_x, vel_x, coeff);
				/*

...
public:
	void
	testSimpleProjection2()
	{
		printf("\n");
		const double p3d_z      = 10.;
		const int    p3d_c      = 8;
		CvPoint3D64f p3d[p3d_c] = {{1, 1, p3d_z + 1},
		                           {1, -1, p3d_z + 1},
		                           {-1, -1, p3d_z + 1},
		                           {-1, 1, p3d_z + 1},
		                           {1, 1, p3d_z - 1},
		...
		for (int i = 0; i < p3d_c; i++) {
			...
			camera.ProjectPoints(vec3d, &P0, vec2d);
			p2d_0 = vec2d[0];
			camera.ProjectPoints(vec3d, &P1, vec2d);
			p2d_1 = vec2d[0];
			camera.ProjectPoints(vec3d, &P2, vec2d);
			p2d_2 = vec2d[0];
	...
	testSimpleProjection()
	{
		...
		camera.ProjectPoints(vec3d, &p2, vec2d);
		p2d_2 = vec2d[0];
		camera.ProjectPoints(vec3d, &p3, vec2d);
		p2d_3 = vec2d[0];

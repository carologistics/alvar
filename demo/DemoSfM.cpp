...
public:
	Visualisator(int n_features) : _n_features(n_features)
	{
		_root      = new osg::Group;
	...
	void
	Update3DPoints(alvar::Pose &pose, std::map<int, alvar::SimpleSfM::Feature> &fm, bool draw_lines)
	{
...
*/
bool
InitVideoCapture(size_t use_cap)
{
	// Enumerate possible capture devices and check the command line argument which one to use
	std::cout << "Enumerated Capture Devices:" << std::endl;
	alvar::CaptureFactory::CaptureDeviceVector vec =
	  alvar::CaptureFactory::instance()->enumerateDevices();
...
*/
bool
InitImages(int w, int h, int origin)
{
	if (w == 0 || h == 0)
		return false;
...
*/
bool
InitOSG(int w, int h, bool flip_image, std::string model_file, std::string camera_file)
{
	int offx = 10;
	int offy = 30;
...
*/
bool
InitALVAR(int w, int h, std::string calib_file, std::string multi_marker_file)
{
...
main(int argc, char **argv)
{
	...
	std::cout << "  This is an example of how to use the 'SimpleSfM' class. 'SimpleSfM' extends"
	          << std::endl;
	std::cout << "  the multi marker tracking. Use 's' to toggle between two possible options:"
	          << std::endl;
	std::cout << "  1) The first option assumes that when the markers are not visible the camera"
	          << std::endl;
	std::cout << "  is only rotating. Image features are used to update rotation parameters."
	          << std::endl;
	std::cout << "  2) The second option uses structure from motion (SfM) technique to estimate all"
	          << std::endl;
	std::cout << "  the camera parameters during tracking. The camera needs to moved in 3D for"
	          << std::endl;
	std::cout << "  this option to work properly." << std::endl;
	std::cout << std::endl;
	std::cout << "  To be able to use this software, one must calibrate the camera using"
	          << std::endl;
	std::cout << "  'SampleCamCalib' included in ALVAR. Additionally the multi marker has to be"
	          << std::endl;
	std::cout << "  defined either manually or using 'SampleMultiMarkerBundle'." << std::endl;
	std::cout << "  The default marker setup 'multimarker.xml' can be printed from 'ALVAR.pdf'."
	          << std::endl;
	std::cout << "  For the best user experience you need to finetune your camera for fast"
	          << std::endl;
	std::cout << "  framerate without motion blur (fast exposure compensated with gain)."
	          << std::endl;
	std::cout << std::endl;
	...
	std::cout << "  s: switch between tracker types (rotation only/structure from motion)"
	          << std::endl;
	...
	if (InitVideoCapture(use_cap) && InitImages(_width, _height, _origin)

...
{
public:
	CSimpleWndSizeHandler(float vX, float vY) : vidX(vX), vidY(vY){};
...
public:
	...
	void
	SetBGImage(osg::Image *image)
	{
		bgTexture->setImage(image);
	};
	osg::Group *
	GetOSGGroup()
	{
		return mMainGroup.get();
	};
...
public:
	...
	void
	AddNode(osg::ref_ptr<osg::Node> node)
	{
		root->addChild(node);
	}
	...
	void
	SetModelMatrix(double gl_mat[16])
	{

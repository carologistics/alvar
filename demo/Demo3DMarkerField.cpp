...
*/
double
GetMultiMarkerPose(cv::Mat &image, alvar::Camera *cam, alvar::Pose &pose)
{
	...
	if (init) {
		...
		std::cout << "MultiMarkerBundle. Point the camera towards the markers 0-" << nof_markers - 1
		          << std::endl;
...
Process()
{
	...
	while (true) {
		...
		if (frame.channels() == 1) {
			...
			cv::cvtColor(frame, rgb, cv::COLOR_BayerGB2RGB); // TODO: Now this assumes Bayer
		} else {
			rgb.data = frame.data;
...
main(int argc, char **argv)
{
	...
	std::cout << "  The program deduces the relative poses and positions of markers that have been"
	          << std::endl;
	std::cout << "  laid in any 3D configuration. Then the program uses the deduced marker"
	          << std::endl;
